		...
		$.ajax(
				...
				success:function(e){
					...
					$.each(e.BrokerLists,function(i,bean){
				    	...
				    	"<dd class='name'><span class='n'>"+bean.name+"</span><button class='msg' title='发消息' type='button'></button>" +
				    	"<span id='sendMsgBrokerList' class='sendMsg round shadow'><span id='num'>还能输入40字</span><a id='sendCloseBrokerList' class='closeBtn'></a><textarea id='sendText' rows='1'></textarea><button id='sendOutBrokerList' class='confirmBtn round' type='button'>发送</button></span></dd>"+
				    	"<dd class='qua'>"+bean.cert+"</dd>"+
				    	"<dd class='tel'>"+bean.mobile+"</dd>"+
				    	"<dd class='store'>"+bean.brokerage_name+"</dd>"+
				    	"</dl>";
						id_index=id_index+1;
