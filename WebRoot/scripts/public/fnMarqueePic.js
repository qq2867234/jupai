function marquees(marqueeDiv,boxWidth){
    if(!boxWidth) boxWidth = 890;
    ...
    function marqThis($this){
        var scrollPic = new Array($(".Marquee").length);
        var marqueeOrder = $this.index(".Marquee");
        var leftBtn = $this.find(".LeftBotton");
        var rightBtn = $this.find(".RightBotton");
        var picsBox = $this.find(".picsBox");
        var picWidth = picsBox.find('li').first().width();
        var picCount = picsBox.find('li').length-1;
        if(($this.is("#RentalChart")||$this.is("#SaleChart"))&&picsBox.find("li").eq(2).children().length<=0)
        {
            leftBtn.hide();
            rightBtn.hide();
            picsBox.addClass('noArr');
        }
        if(picWidth*picCount<=boxWidth)
        {
            leftBtn.hide();
            rightBtn.hide();
            picsBox.addClass('noArr');
            ...
            scrollPic[marqueeOrder] = new ScrollPic();
            scrollPic[marqueeOrder].scrollContId = picsBox.attr("id"); //内容容器ID
            scrollPic[marqueeOrder].arrLeftId = leftBtn.attr("id"); //左箭头ID
            scrollPic[marqueeOrder].arrRightId = rightBtn.attr("id"); //右箭头ID

            scrollPic[marqueeOrder].frameWidth = boxWidth; //显示框宽度
            scrollPic[marqueeOrder].pageWidth = 450; //翻页宽度

            scrollPic[marqueeOrder].speed = 10; //移动速度(单位毫秒，越小越快)
            scrollPic[marqueeOrder].space = 15; //每次移动像素(单位px，越大越快)
            scrollPic[marqueeOrder].autoPlay = false; //自动播放
            scrollPic[marqueeOrder].autoPlayTime = 3; //自动播放间隔时间(秒)

            scrollPic[marqueeOrder].initialize(); //初始化
        }
}
}
