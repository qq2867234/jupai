...
//准备数据
function readyBidListData(bidId, bidderZid, sortType) {
	clearBidListData();
	$.ajaxSettings.async = false;
	$.getJSON("/EntrustingOperator.action?getRestBid",{bidId:bidId, bidderZid:bidderZid, sortType:sortType},function(e){
...
$(function() {
	...
	$(".showMore").click(function() {
		...
		readyBidListData(bidId, bidderZid, sortType);
		parent = $(this).parent();
		parent.children('dl').remove();
		$(this).css('display','none').siblings('.pickup').css('display','block');
		for(var i = 0; i < bidSerialNo.length; i++) {
			...
			for(var j = 0; j < bidImage[i].length; j++) {
				part8 += "<img src=\""+bidImage[i][j].pic + "\" width=\"323\" height=\"220\" alt='实拍照片'/>";
			}
			part8 += "</dd> <dt>补充说明:</dt><dd class='intro'>"+bidContents[i]+"</dd></dl>";
	...
	$(".pickup").click(function() {
		...
		$(this).css('display','none').siblings('.showMore').css('display','block');
	});
