function homePSearch() {
	...
	var budget='';
	var keywords=$("#keywords").val()===undefined?'':clearIgnalCh($("#keywords").val());
	var salebudget=$("#salebudget").val()===undefined?'':$("#salebudget").val().replace(/(^\s*)|(\s*$)/g, "");
	var rentbuduget=$("#rentbudget").val()===undefined?'':$("#rentbudget").val().replace(/(^\s*)|(\s*$)/g, "");
	var bedRoom=$("#RoomCount").val()===undefined?'':$("#RoomCount").val();
	//if($("#secInput").val()=="租房")houseType=2;		
	budget=houseType==1?salebudget:rentbuduget;	
	var url = "/ResidenceSaleSearch.action?toSearcIndex&budget="+formatRent(budget)+"&keywords="+encodeURI(encodeURI(keywords))+"&bedRoom="+bedRoom+"&houseType="+houseType;
 ...
 */
	//买或租
	var houseType=1;
	// $("#secRB .m").click(function(){
	// 	$(this).addClass("on").siblings().removeClass("on");
	// 	$("#secInput").val($(this).text().replace(/(^\s*)|(\s*$)/g, ""));
	
	// 	if($(this).text().indexOf("买房")>=0)
	// 	{
	// 		$(".buyPr").css("display","inline-block");
	// 		$(".rentPr").css("display","none");
	// 	}
	// 	else
	// 	{
	// 		$(".rentPr").css("display","inline-block");
	// 		$(".buyPr").css("display","none");
	// 	}
	// 	});
	$("#gosearh").click(function() {
		homePSearch();
		
	});	
	
	
	$("#salebudget").keyup(function(){
		$("#salebudget").val(formatRent($("#salebudget").val()));
		
	});
	$("#rentbudget").keyup(function(){
		
		$("#rentbudget").val(formatRent($("#rentbudget").val()));
	});
...
// */
//	
//	var brokerList="/BrokerPromotion.action?getBrokers";
//	
//	//获取经纪人
//	$.ajax({
//		url: brokerList,
...
//				
//			content=content+"<a href='"+bean.url+"'>"
//			+"<img src='/account/photo/"+Math.floor(bean.zid/10000)+"/"+bean.pic+"' alt='经纪人头像' />"
//			+"<b class='person'>"
//			+"<em>"+bean.name+"</em>"
//			+"<span>"+(bean.cert == undefined ? '': bean.cert)+"</span>"
//			+"</b>"
//			+"<b class='company'>"
//			+bean.salesofficeName
//			+"</b>"
//			+"<b class='bid'>"
//			+bean.brokerageName
//			+"</b>"
