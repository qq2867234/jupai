$(function() {
	$("#userConnectForm").Validform({
		...
		callback:function(data){
			...
			case '1': // 成功
				...
				break;
			case 'U0001': // 手机号无效
				$("#mobile2").siblings(".Validform_checktip").text(data.message);
				break;
			case '0': // 失败（验证码错误或操作已过期  或 用户不存在 或 已绑定其它账号）
				$(".form-warn").show().children().html(data.message);
				break;
			default:
	...
	new MobileValidCode({
		sendUrl : "/UHomeUser.action?valiSendCode",
...
//			if($("#mobile2").val()==''){
//				$("#mobile2").siblings(".Validform_checktip").html("请输入手机").addClass("Validform_wrong");
//				return false;
//			}
//			if (/^1(3|5|8|4)\d{9}$/.test($("#mobile2").val())) {
...
//				$("#mobile2").siblings(".Validform_checktip").html("").removeClass("Validform_wrong");
//			} else {
//				$("#mobile2").siblings(".Validform_checktip").html("手机格式不正确").addClass("Validform_wrong");
//				return false;
//			}
//			return true;
//		},
//		// 绑定发送短信事件
//		bindSendMessageEvent : function(btn) {
//			var sendBtn = btn || $("#sendMobileValidCode2");
...
//					$.ajax({
//						url: '/UHomeUser.action?valiSendCode',
//						data: {
//							mobile: $("#mobile2").val()
//						},
//						dataType: "json",
